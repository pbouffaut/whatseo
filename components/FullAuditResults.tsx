...
import { Check, AlertTriangle, FileText, Globe, BarChart3, Code, Zap, Bot, Search, Download, ArrowLeft } from 'lucide-react';
...
export default function FullAuditResults({ audit, results }: FullAuditResultsProps) {
  ...
  return (
    <div>
      ...
      {/* Tabs — Rounded pills with tonal background */}
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium whitespace-nowrap transition-all ${
              activeTab === tab.key
                ? 'bg-gradient-cta text-on-primary'
      ...
      <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            {/* Top Priority — sunset gradient accent */}
            ...
            {/* Executive Summary — White card on cream */}
            {insights?.executive && (
            ...
            {issues.length > 0 && (
              <div className="bg-surface-white rounded-[2rem] shadow-ambient p-8">
                <h3 className="font-serif text-xl text-error mb-4">Top Issues ({issues.length})</h3>
            ...
            {/* Category Insights — Grid of tonal cards */}
            {insights && (
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={key} className="bg-surface-white rounded-2xl shadow-ambient p-6">
                    ...
                    <p className="text-on-surface-muted text-xs leading-relaxed">{insights[key].split('\n\n')[0]}</p>
                  </div>
                ) : null)}
              </div>
            )}
              ...
              <div className="grid md:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={p.label} className="flex justify-between items-center py-2.5 px-4 rounded-xl bg-surface-low">
                    ...
                    <span className={`text-sm font-bold ${p.value > 0 && p.bad ? 'text-error' : 'text-tertiary'}`}>
                      {p.value}
                    </span>
                  </div>
                ))}
                  ...
                  {pages.map((page, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-surface-low/50 transition-colors" style={{ borderBottom: '1px solid rgba(219,194,176,0.1)' }}>
                        <td className="px-4 py-3 text-on-surface truncate max-w-[300px]" title={String(page.url)}>
                          {urlPath}
                        </td>
                        <td className="text-center px-3 py-3">
                          <span className={Number(page.statusCode) < 400 ? 'text-tertiary' : 'text-error'}>
                            {String(page.statusCode)}
                          </span>
                        </td>
                        <td className="text-center px-3 py-3 text-on-surface-muted">
                        ...
                        <td className="text-center px-3 py-3">
                          <span className={pageIssues > 0 ? 'text-error font-medium' : 'text-tertiary'}>
                            {pageIssues}
                          </span>
                        </td>
                      </tr>
          ...
          <div className="space-y-6">
            ...
            {ga4 && (
              <div className="bg-surface-white rounded-[2rem] shadow-ambient p-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div className="text-center p-4 bg-surface-low rounded-2xl">
                    <div className="text-xl font-bold text-primary">{String((ga4 as Record<string, unknown>).organicSessions || 0)}</div>
                    <div className="text-xs text-on-surface-light">Organic Sessions</div>
                  </div>
                  <div className="text-center p-4 bg-surface-low rounded-2xl">
                    <div className="text-xl font-bold text-on-surface">{Number((ga4 as Record<string, unknown>).organicPercentage || 0).toFixed(1)}%</div>
                    <div className="text-xs text-on-surface-light">Organic Share</div>
                  </div>
                  <div className="text-center p-4 bg-surface-low rounded-2xl">
                    <div className="text-xl font-bold text-on-surface">{Number((ga4 as Record<string, unknown>).engagementRate || 0).toFixed(1)}%</div>
                    <div className="text-xs text-on-surface-light">Engagement Rate</div>
                  </div>
                  <div className="text-center p-4 bg-surface-low rounded-2xl">
                    <div className="text-xl font-bold text-on-surface">{String((ga4 as Record<string, unknown>).totalSessions || 0)}</div>
                    <div className="text-xs text-on-surface-light">Total Sessions</div>
                  </div>
                </div>

                {((ga4 as Record<string, unknown>).topLandingPages as Record<string, unknown>[])?.length > 0 && (
                  <>
                    <h4 className="text-sm font-medium text-on-surface-muted mb-2">Top Organic Landing Pages</h4>
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="bg-surface-low">
                            <th className="text-left text-on-surface-light px-3 py-2 rounded-l-lg">Page</th>
                            <th className="text-right text-on-surface-light px-3 py-2">Sessions</th>
                            <th className="text-right text-on-surface-light px-3 py-2 rounded-r-lg">Users</th>
                          </tr>
                        </thead>
                        <tbody>
                          {((ga4 as Record<string, unknown>).topLandingPages as Record<string, unknown>[])?.slice(0, 10).map((p, i) => (
                            <tr key={i} style={{ borderBottom: '1px solid rgba(219,194,176,0.1)' }}>
                              <td className="px-3 py-2 text-on-surface truncate max-w-[250px]">{String(p.page)}</td>
                              <td className="px-3 py-2 text-right text-on-surface-muted">{String(p.sessions)}</td>
                              <td className="px-3 py-2 text-right text-on-surface-muted">{String(p.users)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
            ...
            {!gsc && !ga4 && (
              <div className="text-center py-12">
                ...
                <p className="text-on-surface-muted mb-2">No Google data available for this audit</p>
                <p className="text-on-surface-light text-xs">Connect Google Search Console and GA4 in Settings to get real traffic data.</p>
                <Link href="/onboarding" className="text-primary text-sm hover:text-primary-container mt-4 inline-block">
                  Connect now →
                </Link>
              </div>
            )}
        ...
        {activeTab === 'performance' && (
          <div className="space-y-6">
                  ...
                  {[
                    ...
                    return (
                      <div key={metric.label} className="flex items-center justify-between py-3 px-4 rounded-xl bg-surface-low">
                        ...
                        <span className="text-xs text-on-surface-light">{good}% good</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {!psi && !crux && (
              <p className="text-on-surface-muted text-center py-8">No performance data available for this audit.</p>
            )}
          </div>
        )}
      ...
      <div className="flex justify-center gap-4 mt-12">
        ...
        <Link href="/dashboard" className="inline-flex items-center gap-2 px-8 py-3.5 bg-surface-high text-on-surface-muted rounded-full font-semibold hover:bg-surface-highest transition-colors">
