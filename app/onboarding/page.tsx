...
export default function OnboardingPage() {
  ...
  const handleConnectGSC = () => {
    // Direct Google OAuth — bypasses Supabase's unreliable token handling
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      // Load onboarding data from DB (tokens already saved by /api/google/callback)
      const { data } = await supabase.from('onboarding_data').select('*').eq('user_id', user.id).single();

      if (data) {
        setForm({
          websiteUrl: data.website_url === '_pending_' ? '' : (data.website_url || ''),
    ...
    <div className="min-h-screen pt-28 pb-16 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            <FieldLabel label="Google Search Console &amp; Analytics"
              description="Connects your real click, impression, and ranking data from Search Console, plus organic traffic data from GA4. We request read-only access. You can revoke anytime from your Google account." />
            {form.gscConnected ? (
              <div className="flex items-center gap-3 px-5 py-3 rounded-xl bg-[#4aab6a]/10 border border-[#4aab6a]/20">
                ...
                <span className="text-[#4aab6a] text-sm font-medium">Google connected</span>
              ...
              <button type="button" onClick={handleConnectGSC}
          ...
          {/* GA4 Property ID */}
          <div>
            <FieldLabel label="Google Analytics Property ID"
            ...
            <input type="text" value={form.ga4PropertyId}
          ...
          {/* Competitor URLs */}
          <div>
            <FieldLabel label="Competitor URLs (up to 3)"
